...
1. Execute the following command to publish the application and its dependencies to a folder for deployment:
    ...
    ````
    > **NOTE:** Before executing this command, the `<OUTPUT_DIRECTORY>` path must exist. The `-o` command option indicates the output directory. For more information on this command read this [documentation](https://docs.microsoft.com/en-us/dotnet/core/tools/dotnet-publish#description).
